  ...
  if (!messages.length) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center text-cream/60 text-sm px-6 py-8">
        ...
        <p className="text-center text-cream/80 font-medium font-display text-lg">Bonjour! I'm Remy 👨‍🍳</p>
    ...
    <div ref={messagesContainerRef} className="flex-1 overflow-y-auto">
      {messages.map(({ id, role, parts }) => (
        <div
          key={id}
          className={`py-3 ${
            role === "assistant" ? "bg-gradient-to-r from-copper/5 via-gold/5 to-copper/5" : "bg-transparent"
          }`}
        >
      ...
      <div className="relative flex items-center justify-between p-4 border-b border-copper/10">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {contextTitle && <p className="text-xs text-copper/70 truncate max-w-[220px]">🥐 {contextTitle}</p>}
          </div>
        </div>
        <button
