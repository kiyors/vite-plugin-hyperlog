  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-6 py-4">
        <Link to="/talks" className="inline-flex items-center gap-2 text-cream/60 hover:text-gold transition-colors">
      ...
      <div className="max-w-4xl mx-auto px-6">
        ...
        {/* Title */}
        <h1 className="font-display text-4xl md:text-5xl font-bold text-cream mb-6 leading-tight">{talk.title}</h1>
          ...
          {speaker ? (
            <Link to={`/speakers/${speaker.slug}`} className="flex items-center gap-3 group">
              <div className="w-12 h-12 rounded-full overflow-hidden border border-border/50">
                <img src={`/${speaker.headshot}`} alt={speaker.name} className="w-full h-full object-cover" />
              </div>
              <div>
                <p className="text-cream group-hover:text-gold transition-colors font-medium">{talk.speaker}</p>
